...
const deleteBoard = ctrlWrapper(async (req, res) => {
    ...
    try {
        const columns = await Column.find({ board: id }).session(session);
        for (const column of columns) {
            await Card.deleteMany({ column: column._id }).session(session);
        }
        await Column.deleteMany({ board: id }).session(session);
        const deletedBoard = await Board.findByIdAndDelete(id).session(session);
        if (!deletedBoard) {
            throw HttpError(404, `Board with id ${id} not found`);
        }
        await User.findByIdAndUpdate(_id, { activeBoard: null }).session(session);
        await session.commitTransaction();
        session.endSession();
        res.json({ id, message: `Board with id ${id} deleted successfully` });
    } catch (error) {
